...
Ralio.prototype.setTaskState = function (task_id, options, callback) {
  ...
  this.bulk(query, function (error, data) {
    if (error !== null) {
      ...
      if (options.state) {
        if (type === "DE" && (options.state === "In-Progress" || options.state === "Defined"))
          options.state = "Open";
      ...
      if (options.own) {
        taskObject.Owner = user._ref;
      } else {
        taskObject.Owner = null;
      }

      if (options.remaining !== undefined) {
        taskObject.ToDo = options.remaining;
      }
        ...
        if (error) {
          ...
          self.bulk(query, function (error, data) {
            if (error) {
              ...
              var data = data.defect || data.task;
  ...
  this.bulk(
    ...
    function (error, data) {
      if (error !== null) {
        ...
        defects = _.filter(defects, function (story) {
          return _.any(defects, function (defect) {
            return defect.Owner !== null &&
              defect.State === 'Open' &&
              defect.Owner._ref == user._ref;
          });
        });
