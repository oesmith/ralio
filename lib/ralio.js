...
Ralio.prototype.request = function(options, callback) {
  request(options, function (error, response, data) {
    if (error !== null) {
      ...
      callback("Authentication failed! Check your ~/.raliorc file!".red);
    }
...
Ralio.prototype.task = function (option, projectName, storyID, taskName, callback) {
  var self = this;

  switch(option){
    case "create": 
      self.getStory(storyID, function(story){
        self.getProject(projectName, function(project){
    ...
    break;
    case "delete": 
      self.getTask(storyID, function(task) {
        ...
        self.request(options, function(error, data){
          callback(storyID.yellow + " deleted".green);
        });
      });
...
Ralio.prototype.getTask = function (taskID, callback){
  ...
  var options = {
        url: this.bulkUrl({"pathname":"/slm/webservice/1.36/task.js?query=(FormattedID = " + taskID + ")"}),
        method: 'GET',
        json: {},
        strictSSL: !Ralio.test
      };
  ...
  this.request(options, function(error, data){
    resource = data.QueryResult.TotalResultCount === 0 ? 
                  "Task Not Found!".red : data.QueryResult.Results[0];
    callback(resource);
  });

};

Ralio.prototype.getStory = function (storyID, callback){
  var query = {query: '(FormattedID = "' + storyID + '")'};
  this.bulk({defect: query, hierarchicalrequirement: query}, function (error, data) {
    if (error !== null) {
      callback(error);
    } else {
      resource = data.hierarchicalrequirement.TotalResultCount === 0 ? 
                    "Story Not Found!".red : data.hierarchicalrequirement.Results[0];
      callback(resource);
    }
  });
...
Ralio.prototype.getProject = function (projectName, callback){

 var options = {
    url: this.bulkUrl({"pathname":"/slm/webservice/1.36/project.js?query=(Name = " + projectName + ")"}),
    method: 'GET',
    json: {},
    strictSSL: !Ralio.test
  };
