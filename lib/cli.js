...
CLI.prototype.start = function (task) {
  var self = this;
  this.ralio.setTaskState(this.fetchID(task), {state: 'In-Progress', own: true, blocked: false}, function (error, task) {
    self.errors(error);
...
CLI.prototype.finish = function (task) {
  var self = this;
  this.ralio.setTaskState(this.fetchID(task), {state: 'Completed', own: true, blocked: false}, function (error, task) {
    self.errors(error);
    self.printTaskLine(task, {quickid: false});
  });
};
...
CLI.prototype.abandon = function (task) {
  var self = this;
  this.ralio.setTaskState(this.fetchID(task), {state: 'Defined', own: false, blocked: false}, function (error, task) {
    self.errors(error);
    self.printTaskLine(task, {quickid: false});
  });
};
...
CLI.prototype.task = function (option, story, opts) {
  ...
  this.ralio.task(option, projectName, this.fetchID(story), opts.name, function (error, option, story, taskname) {
    self.errors(error);
    self.printStoryLine(story, {quickid: false});
  });
};
