...
CLI.prototype.start = function (task, opts) {
  var self = this;
...
CLI.prototype.finish = function (task, options) {
  var self = this,
      update_options = {state: 'Completed', own: true, blocked: false};
  ...
  this.ralio.setTaskState(this.fetchID(task), options, function (error, task) {
    self.errors(error);
    self.printTaskLine(task, {quickid: false});
  });
};
